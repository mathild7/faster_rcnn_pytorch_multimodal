...
class MultiScaleRoIAlign(nn.Module):
    ...
    def forward(self, x, boxes, image_shapes):
        ...
        for level, (per_level_feature, scale) in enumerate(zip(x_filtered, scales)):
            idx_in_level = (levels == level).to(torch.int64)
            idx_in_level = idx_in_level.nonzero()
            idx_in_level = idx_in_level.view(-1)
            rois_per_level = rois[idx_in_level]
            #result_idx_in_level = RoIAlign(self.output_size, scale, self.sampling_ratio)(per_level_feature, rois_per_level)
